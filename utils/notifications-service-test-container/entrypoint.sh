#!/bin/sh
# Send one notification to the notifications-service and append the returned latency to /output/response.txt.

resp=$(curl -s -X POST 'http://notifications-service:3000/api/notify' \
